		...
		if(o->datac != MINE_COMMAND) {
			...
			case MINE_STEP:
				MechLOSBroadcast(mech,
								 tprintf
								 ("moves to %d,%d, and triggers a mine!", x,
								  y));
				mech_printf(mech, MECHALL,
							"As you move to %d,%d, you trigger a mine!", x,
							y);
				break;
			case MINE_LAND:
	...
	for(o = map->mapobj[TYPE_MINE]; o; o = o2) {
		...
		if(o->x == x && o->y == y) {
			...
			if(o->datac == MINE_TRIGGER) {
				...
				if(nearbyintf(FindHexRange(x1, y1, x2, y2)) >
				   ((float) o->datai))
					continue;
	...
	if(type == MINE_TRIGGER) {
		...
		for(x1 = x - data; x1 <= x + data; x1++)
			for(y1 = y - data; y1 <= y + data; y1++) {
				...
				if(nearbyintf(FindHexRange(fx, fy, fx1, fy1)) <=
				   ((float) data))

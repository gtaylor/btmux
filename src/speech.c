...
void do_page(dbref player, dbref cause, int key, char *tname, char *message)
{
	...
	if(((target = lookup_player(player, tname, 1)) == NOTHING) && n > 1) {
		...
		for(p = (char *) strtok(tname, " "); p != NULL;
			...
			if(target == NOTHING) {
				...
				case ':':
					notify_with_cause(target, player,
									  tprintf("From afar, to (%s):%s %s %s",
											  buf1, aladd, Name(player),
											  message + 1));
					break;
				case ';':
					message++;
				...
				default:
					notify_with_cause(target, player,
									  tprintf("To (%s), %s%s pages you: %s",
											  buf1, Name(player), aladd,
											  message));
				}
	...
	*(bp2 - 1) = '\0';
	...
	if(count == 1) {
		if(*buf1) {
			if(ispose != 1) {
				notify_printf(player, "You paged %s with '%s'.", buf1, mp);
			} else {
				if(mp[0] == ':')
		...
		case PEMIT_WHISPER:
			...
			default:
				notify_printf(player, "You whisper \"%s\" to %s.",
							  message, Name(target));
				notify_with_cause(target, player,
								  tprintf("%s whispers \"%s\"", Name(player),
										  message));
			}

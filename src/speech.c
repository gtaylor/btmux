...
void do_say(dbref player, dbref cause, int key, char *message)
{
	...
	break;
    case SAY_POSE:
	notify_all_from_inside(loc, player, tprintf("%s %s", Name(player),
		message));
	break;
    case SAY_POSE_NOSPC:
	notify_all_from_inside(loc, player, tprintf("%s%s", Name(player),
    ...
    const char *dflt)
{
	...
	*bp = '\0';
	if (*str2) {
	    t = time(NULL);
	    tp = localtime(&t);
	    if (Wizard(target) || !In_IC_Loc(target))
		notify_with_cause(player, target,
		    tprintf("%s message from %s: %s", tag, Name(target),
			str2));
	    notify_with_cause(target, player,
		tprintf("[%d:%02d] %s message sent to %s.", tp->tm_hour,
		    tp->tm_min, tag, Name(player)));
	}
	free_lbuf(str2);
    } else if (dflt && *dflt) {
...
static int page_check(dbref player, dbref target)
{
    ...
    if (!could_doit(player, target, A_LPAGE) || (!Wizard(player) &&
	    In_IC_Loc(target) && !Wizard(target))) {
    ...
    int ismessage)
{
	...
	if (ismessage)
		...
		safe_str(tprintf("%s, ", Name(lookup_player(player, p,
				1))), namelist, &bp);
	    }
	}
    }
...
void do_page(dbref player, dbref cause, int key, char *tname, char *message)
{
     ...
     * Count the words 
     */
    for (n = 0, str = tname; str;
	str = (char *) next_token(str, ' '), n++);
	...
	if (target == NOTHING) {
	    ...
	    case ':':
		notify_with_cause(target, player,
		    tprintf("From afar,%s %s %s", aladd, Name(player),
			message + 1));
		break;
	    case ';':
		message++;
		...
		message++;
	    default:
		notify_with_cause(target, player, tprintf("%s%s pages: %s",
			Name(player), aladd, message));
    ...
    char *message)
{
	...
	case PEMIT_WHISPER:
		...
		if (loc != NOTHING) {
		    ...
		    safe_str((char *) " whispers something to ", buf2,
			&bp);
	...
	case PEMIT_FSAY:
	    ...
	    break;
	case PEMIT_FPOSE:
	    notify_all_from_inside(loc, player, tprintf("%s %s",
		    Name(target), message));
	    break;
	case PEMIT_FPOSE_NS:
	    notify_all_from_inside(loc, player, tprintf("%s%s",

...
static void telnet_send_subop(DESC *client, int option, int how) {
    unsigned char buffer[16] = { TELNET_IAC, TELNET_SB, option, how, TELNET_IAC, TELNET_SE };
...
static int telnet_handle_subop(DESC *client, unsigned char *buffer,
    ...
    switch(buffer[0]) {
       case TELNET_OPT_TERMTYPE:
           ...
           for(iter = 2; iter < length && iter < 18 && buffer[iter] != 0xFF; iter++) {
...
void telnet_read_ring(DESC *client) {
    ...
    dprintk("Ring Read started, input length = %d, ringhead = %d, ringtail = %d. ", 
            ring_length(client), client->ringhead, client->ringtail);
    
    while(ring_length(client) > 0) {
        dprintk("Parsing data 0x%02x,  %d bytes in ring, ringhead = %d, ringtail = %d", 
                ring_char(client, 0), ring_length(client), client->ringhead, client->ringtail); 
        switch(ring_char(client, 0)) {
            case TELNET_IAC:
                if(ring_length(client) < 2) return;
                ...
                switch((unsigned char)ring_char(client, 1)) {
                    ...
                    case TELNET_SB:
                        complete = 0;
                        for(iter = 2; !complete && iter < ring_avail(client);
                                iter++) {
    ...
    dprintk("Ring Read finished. RingState = { head = %d, tail = %d }, InputState = { tail = %d }",

...
int utf8_test_valid(const unsigned char *input, const unsigned int length)
{
	...
	for(current = 0; current < length; current++) {
		...
		case 0xC2...0xDF:
			if(remaining < 1)
				return -1;
			if(input[current + 1] < 0x80 || input[current + 1] > 0xBF)
				return 0;
			current += 1;
			break;
		case 0xE0:
			if(remaining < 2)
				return -(remaining);
			if(input[current + 1] < 0xA0 || input[current + 1] > 0xBF)
				return 0;
		case 0xE1...0xEF:
			if(remaining < 2)
				return -(remaining);
			if(input[current + 1] < 0x80 || input[current + 1] > 0xBF)
				return 0;
			if(input[current + 2] < 0x80 || input[current + 2] > 0xBF)
				return 0;
			current += 2;
			break;
		case 0xF0:
			if(remaining < 3)
				return -(remaining);
			if(input[current + 1] < 0x90 || input[current + 1] > 0xBF)
				return 0;
		case 0xF1...0xF7:
			if(remaining < 3)
				return -(remaining);
			...
			if(input[current + 2] < 0x80 || input[current + 2] > 0xBF)
				return 0;
			if(input[current + 3] < 0x80 || input[current + 3] > 0xBF)
				return 0;
			current += 3;
			break;
		case 0xF8:
			if(remaining < 4)
				return -(remaining);
			if(input[current + 1] < 0x88 || input[current + 1] > 0xBF)
				return 0;
		case 0xF9...0xFB:
			if(remaining < 4)
				return -(remaining);
			...
			if(input[current + 3] < 0x80 || input[current + 3] > 0xBF)
				return 0;
			if(input[current + 4] < 0x80 || input[current + 4] > 0xBF)
				return 0;
			current += 4;
			break;
		case 0xFC:
			if(remaining < 5)
				return -(remaining);
			if(input[current + 1] < 0x84 || input[current + 1] > 0xBF)
				return 0;
		case 0xFD:
			if(input[current + 1] < 0x80 || input[current + 1] > 0xBF)
				return 0;
			...
			if(input[current + 4] < 0x80 || input[current + 4] > 0xBF)
				return 0;
			if(input[current + 5] < 0x80 || input[current + 5] > 0xBF)
				return 0;
			current += 5;
			break;
			/* multibyte */
...
main()
{
	...
	for(i = 0; i < sizeof(testStrings) && testStrings[i] != NULL; i++) {
		printf(" %d = %d\n", i,
			   utf8_test_valid(testStrings[i], strlen(testStrings[i])));
	}
}

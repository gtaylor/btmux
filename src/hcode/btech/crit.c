    ...
    int critHit, int critType, int LOS)
{
    ...
    /* Find the first crit */
    wFirstCrit =
	FindFirstWeaponCrit(wounded, hitloc, critHit, 0, critType,
	wMaxCrits);
	...
	if (FindAmmoForWeapon_sub(wounded, -1, -1, Weapon2I(critType),
	    ...
	    if (attacker) {
		DamageMech(wounded, attacker, 0, -1, hitloc, 0, 0, 0,
		    MechWeapons[Weapon2I(critType)].damage, -1, 7, -1, 0,
		    1);
    ...
    int LOS)
{
    ...
    if (handleWeaponCrit(objAttacker, objMech, wLoc, wCritNum, wCritType,
	    LOS)) {
	return;
    }
...
void DoVehicleEngineHit(MECH * objMech, MECH * objAttacker)
{
    ...
    mech_notify(objMech, MECHALL,
	"%ch%crYour engine takes a direct hit!%cn");
...
void DoVehicleSensorCrit(MECH * objMech)
{
    ...
    mech_notify(objMech, MECHALL,
	"%ch%crYour sensor suite takes a hit!%cn");
    MechBTH(objMech) += 1;
}
    ...
    int critHit, int critType, int critData)
{
    ...
    int tLocIsLeg = ((hitloc == LLEG || hitloc == RLEG) || ((hitloc == LARM
		|| hitloc == RARM) && MechIsQuad(wounded)));
    char partBuf[100];
	...
	scoreEnhancedWeaponCriticalHit(mech, attacker, LOS, hitloc,
	    critHit);
	...
	case HEAT_SINK:
	    ...
	    if (!Destroyed(wounded)) {
		sprintf(msgbuf, "'s %s is covered in a green mist!",
		    locname);
		MechLOSBroadcast(wounded, msgbuf);
	    }
	    break;
    ...
    int num)
{
	...
	if (mudconf.btech_fasaadvvhlcrit) {
	    for (i = 0; i < num; i++)
		HandleAdvFasaVehicleCrit(wounded, attacker, LOS, hitloc,
		    num);

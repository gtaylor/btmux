...
void muxevent_tickmech_removesection(MUXEVENT * e)
{
    ...
    AddPartsM(mech, loc, ProperInternal(mech), 0, (2 * GetSectInt(mech,
		loc)) / extra);
    AddPartsM(mech, loc, ProperArmor(mech), 0, (2 * GetSectArmor(mech,
		loc)) / extra);
    AddPartsM(mech, loc, Cargo(S_ELECTRONIC), 0, (GetSectInt(mech,
		loc)) / extra);
#endif
...
void muxevent_tickmech_repairarmor(MUXEVENT * e)
{
	...
	if (loc >= 8) {
		...
		MechStatus(mech) &= ~DESTROYED;
		mech_printf(mech, MECHALL,
		    "%s's armor repairs have been finished.",
			buf);
		if (i)
		    MechStatus(mech) |= DESTROYED;
	    } while (0);
...
void muxevent_tickmech_reattach(MUXEVENT * e)
{
	...
	if (Destroyed(mech))
	    i = 1;
	MechStatus(mech) &= ~DESTROYED;
	mech_printf(mech, MECHALL, "%s has been reattached.",
		buf);
	if (i)
	    MechStatus(mech) |= DESTROYED;
    } while (0);

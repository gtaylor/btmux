...
/*
 * $Id: map.c,v 1.4 2005/08/08 07:51:20 murrayma Exp $
 *
...
void map_view(dbref player, void *data, char *buffer)
{
    ...
    maptext =
        MakeMapText(player, NULL, mech_map, x, y, MAP_DISPLAY_WIDTH,
                displayHeight, 3, 0);
    for (i = 0; maptext[i]; i++)
    ...
    if (!(muxevent_tick % 60)) {
        ...
        if (ml != oldl || ma != oldv) {
            for (i = 0; i < map->first_free; i++) {
                ...
                if (!(mech = getMech(j))) {
                    snprintf(buffer, MBUF_SIZE, "hag] kicked nonexistent mech #%d off of map #%d.", 
                            map->mechsOnMap[i], map->mynum);
                    SendError(buffer);
                    map->mechsOnMap[i] = -1;
                    continue;
                }

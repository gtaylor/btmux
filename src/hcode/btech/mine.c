...
/*
 * $Id: mine.c,v 1.1 2005/06/13 20:50:50 murrayma Exp $
 *
    ...
    int reason)
{
	...
	if (o->datac != MINE_COMMAND) {
		...
		mech_notify(mech, MECHALL,
		    tprintf("As you move to %d,%d, you trigger a mine!", x,
			y));
		break;
	    case MINE_LAND:
...
static void possible_mine_explosion(MECH * mech, MAP * map, int x, int y,
    int reason)
{
    mapobj *o, *o2;
...
void possible_mine_poof(MECH * mech, int reason)
{
    ...
    if (!is_mine_hex(map, x, y))
	return;
    if (MechZ(mech) > (MechRTerrain(mech) == ICE ? 0 : Elevation(map, x,
		y)))
	return;
...
static void add_mine_on_map(MAP * map, int x, int y, char type, int data)
{
    ...
    if (type >= MINE_LOW && type <= MINE_HIGH) {
	if (VIBRO(type) && mdis) {
...
#define READINT(from,to) \
    ...
    DOCHECK((type =
	    compare_array(mine_type_names, args[2])) < 0,
	"Invalid mine type!");
    ...
    add_mapobj(map, &map->mapobj[TYPE_MINE], &foo, 1);
    notify(player,
	tprintf("%s mine added to (%d,%d) (strength: %d / extra: %d)",
    ...
    int y)
{
	...
	"You see nothing else of interest in the hex, either.");
    mech_notify(mech, MECHALL,
	"Small bomblets litter the hex, interesting... You vaguely recall them from some class or other.");
}

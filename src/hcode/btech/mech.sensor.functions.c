...
/*
 * $Id: mech.sensor.functions.c,v 1.1 2005/06/13 20:50:50 murrayma Exp $
 *
...
/* Seismic sees, as long as there is a target, and it isn't jumping or
   ...
   ground. Period. */
CSEEFUNC(seismic_csee, !(map->sensorflags & (1 << SENSOR_SE)) &&
        t && !Jumping(m) &&
	((MechMove(m) != MOVE_VTOL) || ((MechMove(m) == MOVE_VTOL) &&
		Landed(m))) && ((MechMove(m) != MOVE_FLY) ||
	    ((MechMove(m) == MOVE_FLY) && Landed(m))) && Started(t) &&
	!Jumping(t) && (MechType(t) != CLASS_BSUIT) &&
	(MechType(t) != CLASS_MW) && (MechMove(t) != MOVE_HOVER) &&
...
CSEEFUNC(radar_csee, !(map->sensorflags & (1 << SENSOR_RA)) &&
    t && MechZ(t) > 2 && !(f & MECHLOSFLAG_BLOCK) &&
    (MechZ(t) >= 10 || (r < (MechZ(t) * MechZ(t)))) &&
    MechsElevation(t) > 1);

CSEEFUNC(bap_csee, !(map->sensorflags & (1 << SENSOR_BAP)) &&
    !AnyECMDisturbed(m) && t && !AngelECMProtected(t) &&
    !StealthArmorActive(t) && !NullSigSysActive(t));

CSEEFUNC(blood_csee, !(map->sensorflags & (1 << SENSOR_BHAP)) &&
    !AnyECMDisturbed(m) && t && !AngelECMProtected(t));

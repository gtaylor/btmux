...
/*
 * $Id: autopilot.c,v 1.1 2005/06/13 20:50:49 murrayma Exp $
 *
...
PSTART(a,mech);\
if (MechType(mech) == CLASS_MECH && Fallen(mech) && !(CountDestroyedLegs(mech) > 0)) { if (!Standing(mech)) mech_stand(a->mynum, mech, my2string("")); REDO(a, AUTOPILOT_NC_DELAY); return; };\
if (MechType(mech) == CLASS_VTOL && Landed(mech) && !SectIsDestroyed(mech, ROTOR)) { if (!TakingOff(mech)) aero_takeoff(a->mynum, mech, my2string("")); REDO(a, AUTOPILOT_NC_DELAY); return; }
...
void auto_com_event(EVENT * e)
{
    ...
    case GOAL_GOTO:
	GSTART(a, mech);
	AUTOEVENT(a, EVENT_AUTOGOTO, auto_goto_event, AUTOPILOT_GOTO_TICK,
	    0);
	return;
    case GOAL_DUMBGOTO:
	GSTART(a, mech);
...
/*       mech_loadcargo(GOD, mech, "50"); */
	...
	PSTART(a, mech);
	autopilot_enterbase(mech, GVAL(a, 1));
	ADVANCE_PG(a);
	return;
    case COMMAND_SPEED:
	a->speed = GVAL(a, 1);
	...
	if (auto_valid_progline(a, GVAL(a, 1))) {
	    PG(a) = GVAL(a, 1);
	    AUTOEVENT(a, EVENT_AUTOCOM, auto_com_event, AUTOPILOT_NC_DELAY,
		0);
	} else {
	    ADVANCE_PG(a);
	}
    ...
    int bearing, int tx, int ty)
{
	...
	mech_speed(a->mynum, mech, tprintf("%.2f",
		(float) (0.4 + range / 2.0) * MMaxSpeed(mech)));
    return 1;
}
...
void auto_dumbgoto_event(EVENT * e)
{
    ...
    update_wanted_heading(a, mech, bearing);
    AUTOEVENT(a, EVENT_AUTOGOTO, auto_dumbgoto_event, AUTOPILOT_GOTO_TICK,
	0);
}
...
void auto_follow_event(EVENT * e)
{
    ...
    FindComponents(MechSpeed(leader) * MOVE_MOD, MechFacing(leader), &newx,
	&newy);
...
void auto_leave_event(EVENT * e)
{
    ...
    num = a->speed;
    a->speed = 100;
    speed_up_if_neccessary(a, mech, 0, 0, dir);
    a->speed = num;
    update_wanted_heading(a, mech, dir);
    AUTOEVENT(a, EVENT_AUTOLEAVE, auto_leave_event, AUTOPILOT_LEAVE_TICK,
	0);
}

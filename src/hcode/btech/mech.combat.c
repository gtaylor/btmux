    ...
    int mapy, float range, int indirectFire, int sight, int ishex)
{
    ...
    DOCHECKMA((wAmmoMode & STINGER_MODE) && target && !(Jumping(target) || OODing(target) || (FlyingT(target) && !Landed(target))),
...
*/
    ...
    if (IsMissile(weapindx)) {
        ...
        mapy, LOS, type, modifier, (roll >= (mudconf.btech_use_glancing_blows ? baseToHit-1 :baseToHit)) &&
        range_ok, baseToHit, wGattlingShots, tIsSwarmAttack, roll);
    } else {
        if (roll >= (mudconf.btech_use_glancing_blows ? baseToHit-1 :baseToHit) ) {
        HitTarget(mech, weapindx, section, critical, target, mapx,

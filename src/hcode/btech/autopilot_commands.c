...
/*
 * $Id: autopilot_commands.c,v 1.2 2005/06/22 22:07:18 murrayma Exp $
 *
...
int auto_valid_progline(AUTO * a, int p)
{
    ...
    for (i = 0; i < a->first_free;
	i += (acom[a->commands[i]].argcount + 1))
	if (i == p)
	    return 1;
    return 0;
}
...
void auto_delcommand(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(Readnum(p, buffer),
	"Invalid argument - single number expected.");
    if (p >= 0)
	...
	if (PG(a) >= p)
	    PG(a) -= len;
	a->first_free -= len;
	for (i = p; i < a->first_free;
	    i += (acom[a->commands[i]].argcount + 1))
	    if (a->commands[i] == COMMAND_JUMP)
		a->commands[i + 1] -= len;
	notify(player, tprintf("Command #%d deleted (%d words freed)!", p,
		len));
    }
}
...
void auto_addcommand(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK((argc =
	    mech_parseattributes(buffer, args, 4)) == 4,
...
void StopAutoPilot(AUTO * a)
{
    ...
    for (i = FIRST_AUTO_EVENT; i <= LAST_AUTO_EVENT; i++)
	muxevent_remote_type_data(i, (void *) a);
}

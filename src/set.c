...
/*
 * $Id: set.c,v 1.2 2005/08/08 09:43:07 murrayma Exp $ 
 */
...
char *name, *newown;
{
	...
	notify_quiet(player, "Players always own themselves.");
		...
		(Location(thing) != player) && !Chown_Any(player))) ||
...
void set_attr_internal(player, thing, attrnum, attrtext, key)
dbref player, thing;
int attrnum, key;
char *attrtext;
{
...
char *newpair[];
{
	...
	if (!oldattr) {
	    ...
	    if (!newattr) {
		do_set(player, cause, 0, newthing, tprintf("%s:_%s/%s",
			oldattr, oldthing, oldattr));
	    } else {
		do_set(player, cause, 0, newthing, tprintf("%s:_%s/%s",
			newattr, oldthing, oldattr));
	    }
	}
    }
}
...
char *it, *args[];
{
	...
	if (ap) {
		...
		if (ap->check != NULL) {
		    doit =
			(*ap->check) (0, player, thing, ap->number,
			result);
		} else {
		    doit = 1;
		}
		if (doit) {
		    atr_add(thing, ap->number, result, Owner(player),
			aflags);
		    if (!Quiet(player))

...
static int network_bind_port(struct sockaddr *addr, int salen) {
    ...
    event_set(&lst->ev, lst->fd, EV_READ | EV_PERSIST, network_accept_client, lst);
...
int network_init(int port) {
    ...
    hints.ai_flags = AI_PASSIVE | AI_NUMERICSERV | AI_ADDRCONFIG | AI_ALL | AI_V4MAPPED;
        ...
        while (walk) {
            ...
            if(network_bind_port(walk->ai_addr, walk->ai_addrlen) == 0) {
                log_printf("network] binding to %s %s %s failed.", 
                        (walk->ai_family == AF_INET6 ? "IPv6" : "IPv4"), hostname, myservice);
            } else {
...
void network_release_socket(DESC *desc) {
    ...
    desc->chokes--;
    if(desc->chokes < 0) desc->chokes = 0;
...
static void network_accept_client(int fd, short event, void *arg) {
    ...
    if(newfd < 0) {
        log_printf("network] accept on %d failed with '%s'", fd, strerror(errno));
        return;
    }
...
DESC *network_initialize_socket(int socket, struct sockaddr *addr, int addrlen) {
    ...
    if(getnameinfo(addr, addrlen, remotehost, NETWORK_HOSTNAME_MAX, 
                remoteport, NETWORK_PORTNAME_MAX, NI_NUMERICSERV | NI_NUMERICHOST)) {
    ...
    desc->sock_buff = bufferevent_new(desc->fd, network_write_callback,
            network_read_callback, network_error_callback, NULL);
...
static void network_client_input(int fd, short event, void *arg) {
    ...
    if(client->ringtail < client->ringhead) {
        ...
        handle_errno(net_length = read(client->fd, 
                    client->ringbuffer+client->ringtail, avail));
    } else {
        if(client->ringhead == 0) {
            ...
            if(net_length && net_length == avail) {
                handle_errno(net_length += read(client->fd,
                        client->ringbuffer, client->ringhead - 1));

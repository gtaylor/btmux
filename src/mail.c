...
void do_mail_fwd(dbref player, char *msg, char *tolist)
{
    ...
    atr_add_raw(player, A_MAILFLAGS, tprintf("%d",
	    (atoi(atr_get_raw(player, A_MAILFLAGS)) | M_FORWARD)));
}
    ...
    const char *subject, int number, mail_flag flags, int silent)
{
	...
	notify(player,
	    "MAIL: You cannot send mail to non-existent people.");
	delete_mail_message(number);
	return;
    }
    tt = time(NULL);
...
int dump_mail(FILE *fp)
{
	...
	if (isPlayer(thing)) {
	    mptr =
		(struct mail *) nhashfind((int) thing,
		&mudstate.mail_htab);
	    if (mptr != NULL)
...
int load_mail(FILE *fp)
{
	...
	if (!nhashfind((int) mp->to, &mudstate.mail_htab)) {
		...
		(struct mail *) nhashfind((int) mp->to,
		    &mudstate.mail_htab); mptr->next != NULL;
		mptr = mptr->next);
	    mptr->next = mp;
	    mp->prev = mptr;
	    mp->next = NULL;
	...
	while (strncmp(nbuf1, "+++", 3)) {
	    number = atoi(nbuf1);
	    new_mail_message(getstring_noalloc(fp, read_new_strings),
		number);
	    fgets(nbuf1, sizeof(nbuf1), fp);
	}
    }
    ...
    int number, mail_flag flags, int silent)
{
    ...
    if (!tolist || !*tolist) {
	notify(player,
	    "MAIL: I can't figure out who you want to mail to.");
	return;
    }
...
void do_malias_create(dbref player, char *alias, char *tolist)
{
	...
	ma_size = MA_INC;
	malias =
	    (struct malias **) malloc(sizeof(struct malias *) * ma_size);
    } else if (ma_top >= ma_size) {
	ma_size += MA_INC;
	nm = (struct malias **) malloc(sizeof(struct malias *) *
	    (ma_size));
...
void malias_read(FILE *fp)
{
	...
	if (!Good_obj(malias[i]->owner) || !Alive(malias[i]->owner)) {
	    if (i != ma_top - 1)
		memcpy(malias[i], malias[ma_top - 1],
		    sizeof(struct malias));;
	    ma_top--;
	}
}
...
static char *make_numlist(dbref player, char *arg)
{
	...
	*tail = 0;
	...
	if (num) {
	    ...
	    if (!temp) {
		notify(player,
		    "MAIL: You can't reply to nonexistent mail.");
		free_lbuf(numbuf);
		return NULL;
	    }
